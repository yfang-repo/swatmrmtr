...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      use parm
      !use pflotran_rxn, only: reaction, option, rt_auxvars
      !use pflotran_rxn, only: global_auxvars, material_auxvars
      use pflotran_rxn, only: pflotran_rxn_react, reaction, option
...
!      real :: thrk5 = 1.047, thrk6 = 1.0, thrs6 = 1.024, thrs7 = 1.0
      real*8 :: src_no3, Cno3, volume, dt_tmp
      real*8 :: imcon(reaction%immobile%nimmobile)
      real*8, dimension(23+100) :: swat_params !100/2 rates for each storage
      real*8, dimension(12) :: C_swat_sp
      real*8, dimension(16) :: ctmp
      integer :: nimb
      real*8 :: date_tmp(12)
      integer :: flux_mon,i_tp
      date_tmp = (/31,59,90,120,151,181,212,243,273,303,334,365/)
      if(iyr == 2012) then
        do i_tp=1,12
          if(i_tp==1) then
            flux_mon = int((iida-1)/date_tmp(i_tp)) + 1
          else
            flux_mon = int((iida-1)/date_tmp(i_tp)+1) + 1
          endif
          if(flux_mon == 1) then
            flux_mon = i_tp
            exit
...
!      jrch = 1
      !print *,'In jrch--',jrch
      nimb = reaction%immobile%nimmobile

!! hourly loop
      do ii = 1, nstep
!       if(jrch == 27 ) then
       if(jrch == 60 .and. ii == 4 ) then
        !print *,'jrch-ii',jrch,ii
       endif
...
!YFang
         if(ii == 1) then
! real*4 to real*8
           imcon(1:nimb) = cimmobile(1:nimb,jrch)
           hhimmobile(1:nimb,ii) = cimmobile(1:nimb,jrch)
         else
           imcon(1:nimb) = hhimmobile(1:nimb,ii-1)
         endif
         wtrtot = wtrin + hrchwtr(ii)
!YFang - concentration from transport
         if (ii == 1) then
     ...
     &                                                          / wtrtot
         co2con = ( discarb(jrch) * hrchwtr(ii))       
     &                                                          / wtrtot
         n2con = ( disnit(jrch) * hrchwtr(ii))       
     &                                                          / wtrtot
         else
         ...
         co2con = (hco2(ii-1) * hrchwtr(ii)) / wtrtot
         n2con = (hn2(ii-1) * hrchwtr(ii)) / wtrtot
         end if
...
! rate constants
         swat_params(8) = Theta(rhoq,thgra,wtmp) !rho
         swat_params(9) = Theta(gra,thrho,wtmp) !mu
         swat_params(15) =  Theta(rs1(jrch),thrs1,wtmp)/hdepth(ii) !sig1Ovd
...
!         if (hbod(ii) < 0.) hbod(ii) = 0.
         swat_params(13) = yy !k1
         swat_params(14) = zz !k3
...
!         if (hdisox(ii) < 0.) hdisox(ii) = 0.
         swat_params(19) = Theta(rk2(jrch),thrk2,wtmp) !k2
         swat_params(20) = soxy
         swat_params(21) = xx !k4d
...
!         if (horgn(ii) < 0.) horgn(ii) = 0.
         swat_params(17) = Theta(rs4(jrch),thrs4,wtmp) !sig4
         swat_params(22) = Theta(bc3(jrch),thbc3,wtmp) !beta3
...
! stoichiometric coefficients
        swat_params(1) = ai4/32.d0*1.d-3  !mg O2/mg alg -> mol/mg alg
        swat_params(2) = ai1/14.d0*1.d-3 !mg N/mg alg -> mol/mg alg
        swat_params(3) = ai2/30.974d0*1.d-3 !mg P/mg alg -> mol/mg alg
        swat_params(4) = p_n !watch for unit consistency
        swat_params(5) = ai3/32.d0*1.d-3 !mg O2/mg alg -> mol/mg alg
        swat_params(6) = ai5/32.d0*14.d0 !mg O2/mg N -> mol O2/mol N
        swat_params(7) = ai6/32.d0*14.d0 !mg O2/mg N -> mol O2/mol N
...
!        if (hnh4(ii) < 0.) hnh4(ii) = 0.
        swat_params(10) = Theta(bc1mod,thbc1,wtmp) !beta1
        swat_params(23) = yy !sig3OVd
...
!        if (hno2(ii) < 0.) hno2(ii) = 0.
        ...
        swat_params(11) = Theta(bc2mod,thbc2,wtmp) !beta2
...
!        if (horgp(ii) < 0.) horgp(ii) = 0.
        ...
        swat_params(12) = Theta(bc4(jrch),thbc4,wtmp) !beta4
        swat_params(16) = Theta(rs5(jrch),thrs5,wtmp) !sig5
...
!        if (hsolp(ii) < 0.) hsolp(ii) = 0.
        swat_params(18) = Theta(rs2(jrch),thrs2,wtmp) / 
     &    (hdepth(ii) *1000.) !sig2OVd
!! end phosphorus calculations
! YFang - Prepare for pfltran reaction
        dt_tmp = thour
        volume = wtrtot 
! imcon - substep concentration calculated from pflotran
        C_swat_sp(1) = nh3con 
        C_swat_sp(2) = o2con 
        C_swat_sp(3) = no3con
        C_swat_sp(4) = no2con 
        ...
        C_swat_sp(8) = solpcon 
        C_swat_sp(9) = cbodcon 
!Fang- doc as function of flow rate
        C_swat_sp(10) = 0.8693*exp(1e-4*hrtwtr(ii) / (idt * 60.)) !m3/s
        C_swat_sp(11) = co2con 
        C_swat_sp(12) = n2con 
! nexss parameters, ch_l2 in km, qs_v is in m3/s (q*l*width)
        num_mrmt = size(qs_v,dim=2)
        do  cnt_mrmt = 1, num_mrmt !
        swat_params(23+cnt_mrmt) = qs_v(jrch,cnt_mrmt)/abs(hharea(ii))
     &    /ch_l2(jrch)/1000.d0  ! vertical exchange rate, 1/s
!        swat_params(24) = qs_v(jrch)/max(1.e-8,xsarea(jrch))  ! vertical exchange rate, 1/s
        swat_params(23+num_mrmt+cnt_mrmt) = 
     &    qs_l(jrch,cnt_mrmt)/abs(hharea(ii))
     &    /ch_l2(jrch)/1000.d0  ! lateral exchange rate, 1/s
!        swat_params(25) = qs_l(jrch)/max(1.e-8,xsarea(jrch))  ! lateral exchange rate, 1/s
        if(ts_v(jrch,cnt_mrmt) < 1.d-10) then
          swat_params(23+2*num_mrmt+cnt_mrmt) = 1.d-20
        else
          swat_params(23+2*num_mrmt+cnt_mrmt) = 
     &    1./ts_v(jrch,cnt_mrmt)/swat_params(23+cnt_mrmt)  ! vol_frac_r2st in vertical
        endif
        if(ts_l(jrch,cnt_mrmt) < 1.d-10) then
          swat_params(23+3*num_mrmt+cnt_mrmt) = 1.d-20
        else
          swat_params(23+3*num_mrmt+cnt_mrmt) = 
     &    1./ts_l(jrch,cnt_mrmt)/swat_params(23+num_mrmt+cnt_mrmt)  ! vol_frac_r2st in lateral
        endif
        enddo
!        if(swat_params(24) > 0) then
!          print *,'jrch-',jrch,ii,swat_params(24:27)
!        endif
!

!YFang - test sensitivity in individual reach
!      if(option%time == 3600*365*2*24.0 .and. jrch == 101) then
!      if(option%time == 3600*365*2*24.0 .and. jrch == 27) then
!        imcon(6) = 3.5e-4
!      endif
       if(ii==1) then
       print *,'jrch-',jrch,swat_params(24:27),
     &    qs_l(jrch,1)/ch_l2(jrch)/1000.d0 ,
     &    hharea(ii)
       endif
        call pflotran_rxn_react(C_swat_sp, swat_params, dt_tmp, 
     &     volume, imcon,num_mrmt)
!        stop
! convert to mg
!          hnh4(ii) = real(Cno3*14.d0*1000.d0)
        hnh4(ii) = real(C_swat_sp(1)*14.d0*1000.d0)
        hdisox(ii) = real(C_swat_sp(2)*32.d0*1000.d0)
        hno3(ii) = real(C_swat_sp(3)*14.d0*1000.d0)
        hno2(ii) = real(C_swat_sp(4)*14.d0*1000.d0)
        horgn(ii) = real(C_swat_sp(5)*14.d0*1000.d0)
! kept in mg
        halgae(ii) = real(C_swat_sp(6))
        horgp(ii) = real(C_swat_sp(7)*30.974*1000.d0)
        hsolp(ii) = real(C_swat_sp(8)*30.974*1000.d0)
! mg O2 for CBOD (biochemical oxygen demand)
        hbod(ii) = real(C_swat_sp(9)*32.0d0*1000.d0)
        hco2(ii) = real(C_swat_sp(11)*44.d0*1000.d0)
        hn2(ii) = real(C_swat_sp(12)*14.d0*1000.d0)

         hchla(ii) = 0.
         hchla(ii) = halgae(ii) * ai0 / 1000.
!! end algal growth 
! convert to real*4
        hhimmobile(1:nimb,ii) = real(imcon(1:nimb))
      else
        ...
        soxy = 0.0
        if(ii>1) then
          hhimmobile(1:nimb,ii) = hhimmobile(1:nimb,ii-1)
        endif
      endif
...
! YFang
! convert to real*8
      cimmobile(1:nimb,jrch) = hhimmobile(1:nimb,nstep)
!      if(hno3(nstep) > cimmobile(6,jrch)*14*1000) then
!        print *,jrch
!      endif
      write(9003,'(3i5,16e15.3)') jrch,iida,iyr,cimmobile(1:nimb,jrch)
      ctmp = sum(hhimmobile,dim=2)/nstep
       
!      write(9003,'(3i5,19e15.3)') jrch,iida,iyr,
!     & sum(hhimmobile,dim=2)/nstep,
!     & (ctmp(2)/swat_params(26)+ctmp(10)/swat_params(27))
!     & /(1.d0/swat_params(26) + 1.d0/swat_params(27)),
!     & (ctmp(6)/swat_params(26)+ctmp(14)/swat_params(27))
!     & /(1.d0/swat_params(26) + 1.d0/swat_params(27)),
!     & (ctmp(8)/swat_params(26)+ctmp(16)/swat_params(27))
!     & /(1.d0/swat_params(26) + 1.d0/swat_params(27))
      if(jrch==93) then
         write(9004,'(3i5,21e15.3)') jrch,iida,iyr,
     & sum(hhimmobile,dim=2)/nstep,
     & sum(0.8693*exp(1e-4*hrtwtr / (idt *60.)))/nstep,
     & sum(hrtwtr)/nstep,
     & (ctmp(2)/swat_params(26)+ctmp(10)/swat_params(27))
     & /(1.d0/swat_params(26) + 1.d0/swat_params(27)),
     & (ctmp(6)/swat_params(26)+ctmp(14)/swat_params(27))
     ...
     & /(1.d0/swat_params(26) + 1.d0/swat_params(27))
      ...
      if(jrch==101) then
         write(9005,'(3i5,18e15.3)') jrch,iida,iyr,
     & sum(hhimmobile,dim=2)/nstep,
     & sum(0.8693*exp(1e-4*hrtwtr / (idt *60.)))/nstep,sum(hrtwtr)/nstep
      endif
      ...
      discarb(jrch) = hco2(nstep)
      disnit(jrch) = hn2(nstep)
      
      if(jrch == 1) option%time = option%time + 24*3600
