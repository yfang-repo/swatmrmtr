...
!YFang
  if (reaction%immobile%nimmobile > 0) then
   ...
   !   if (StringCompare(immobile_constraint%names(iimmobile), &
   ...
   ! enddo
   ! if (found) then
   !   rt_auxvar%immobile(jimmobile) = &
   !      immobile_constraint%constraint_conc(iimmobile)
   ! endif  
   !enddo
   rt_auxvar%immobile(:) = immobile_constraint%constraint_conc(:)
  endif
...
subroutine RReact(rt_auxvar,global_auxvar,material_auxvar,tran_xx_p, &
                  num_iterations_,reaction,option,icvg)
  ! 
...
#include <petsc/finclude/petscsys.h>
  ...
  PetscReal :: work_solution(reaction%ncomp)
...
! YFang save in case of nonconvergence  
  work_solution(1:reaction%naqcomp) = rt_auxvar%pri_molal(1:reaction%naqcomp)
  if (reaction%immobile%nimmobile > 0) then
      work_solution(immobile_start:immobile_end) = &
        rt_auxvar%immobile(1:reaction%immobile%nimmobile)
  endif
  ...
  do
    ...
    residual = (residual-fixed_accum) / option%tran_dt
                         ...
                         ! derivative
!YFang
    if (option%transport%numerical_derivatives ) &
      call RReaction(residual,J,PETSC_FALSE,rt_auxvar,global_auxvar, &
                   material_auxvar,reaction,option)
!YFang
    call RReactionDerivative(residual,J,rt_auxvar,global_auxvar, &
                               material_auxvar,reaction,option)
    
    if (maxval(abs(residual)) < reaction%max_residual_tolerance) exit
!YFang   
    conc(1:reaction%naqcomp) = rt_auxvar%pri_molal
    if (reaction%immobile%nimmobile > 0) then
      conc(reaction%offset_immobile+1: &
                reaction%offset_immobile + reaction%immobile%nimmobile) = &
        rt_auxvar%immobile
    endif
    call RSolve(residual,J,conc,update,reaction%ncomp, &
...
!Fang
    new_solution = max(1.d-30,new_solution)
    ...
    if (num_iterations > 50) then
      ...
      if (num_iterations > 500) then
        ...
        icvg = 1
        rt_auxvar%pri_molal(1:reaction%naqcomp) = work_solution(1:reaction%naqcomp)
        if (reaction%immobile%nimmobile > 0) then
        rt_auxvar%immobile(1:reaction%immobile%nimmobile) = &
           work_solution(immobile_start:immobile_end)
        endif    
...
#include <petsc/finclude/petscsys.h>
  ...
  !if (.not.option%transport%numerical_derivatives .or.  &
  !     .not.option%reaction%numerical_derivatives ) then ! analytical derivative
